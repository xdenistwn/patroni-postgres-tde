...
docker-compose exec -T -u postgres $LEADER pgbackrest --stanza=$STANZA_NAME stanza-create

echo "Checking stanza status..."
docker-compose exec -T -u postgres $LEADER pgbackrest --stanza=$STANZA_NAME check
